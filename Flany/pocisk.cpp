#include "pocisk.h"
#include "odbicie.h" // Używamy nagłówka!
#include <cmath>
#include <iostream>
...
void pocisk::aktualizujFizyke(sf::RenderWindow& okno, przeszkoda& cel, sf::Vector2f grawitacja)
{
    ...
    if (czyLeci)
    {
        // Wywołujemy funkcję z odbicie.cpp
...
void pocisk::odbicie_podloze(sf::RenderWindow* okno)
{
    ...
    if (pozycja.y + lotka.getRadius() >= POZIOM_PODLOGI)
    {
        ...
        // Jeśli prędkość pionowa jest bardzo mała -> toczenie
        if (std::abs(predkosc.y) < precyzja)
        {
            ...
            {
                // ZATRZYMANIE (Zamiast Sleep() po prostu wyłączamy fizykę)
                czyLeci = false;
            }
        }
        lotka.setPosition(pozycja);
